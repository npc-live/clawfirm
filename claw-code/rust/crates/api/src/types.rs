...
/// Token usage reported by the API.  OpenRouter proxied models may send
/// `null` for any of these fields (or add extra unknown fields), so every
/// field is lenient: `null` → 0, unknown keys are silently ignored.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct Usage {
