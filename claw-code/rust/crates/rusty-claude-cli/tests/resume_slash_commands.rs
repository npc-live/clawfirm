...
fn resumed_config_command_loads_settings_files_end_to_end() {
    ...
    fs::create_dir_all(project_dir.join(".clawfirm")).expect("project config dir should exist");
    ...
    fs::write(
        project_dir.join(".clawfirm").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
...
fn resume_latest_restores_the_most_recent_managed_session() {
    ...
    let project_dir = temp_dir.join("project");
    let sessions_dir = project_dir.join(".clawfirm").join("sessions");
