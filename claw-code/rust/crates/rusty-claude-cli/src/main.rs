...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match parse_args(&args)? {
        ...
        CliAction::Serve {
            ...
            allowed_tools,
            permission_mode,
        } => run_serve(model, allowed_tools, permission_mode)?,
...
/// Serve mode: long-lived stdin-driven session for subprocess integration.
///
/// Stdin protocol (JSON, one per line):
///   {"type":"prompt","text":"user message"}
///   {"type":"abort"}
///   {"type":"shutdown"}
///
/// Stdout protocol (NDJSON, reuses existing event format):
///   {"type":"ready","session_id":"...","model":"..."}
///   ...existing text_delta, tool_use, tool_result, usage, message_stop, turn_end, error events...
fn run_serve(
    model: String,
    allowed_tools: Option<AllowedToolSet>,
    permission_mode: PermissionMode,
    ...
    // Emit ready signal so the host process knows we're alive.
    println!(
        "{}",
        json!({
            "type": "ready",
    ...
    for line_result in stdin.lock().lines() {
        let line = match line_result {
            Ok(l) => l,
            Err(e) => {
                // Stdin closed or read error — treat as shutdown.
        ...
        match cmd_type {
            "prompt" => {
                let text = cmd["text"].as_str().unwrap_or("").to_string();
                if text.is_empty() {
                    println!(
                        "{}",
                        json!({"type": "error", "message": "empty prompt text"})
                    );
                    let _ = io::stdout().flush();
                    continue;
                }
                if let Err(e) = cli.run_prompt_ndjson(&text) {
                    println!(
                        "{}",
            ...
            "abort" => {
                // Abort is a no-op in synchronous mode — the turn runs to
                // completion within run_prompt_ndjson. A future async
                // implementation could use an abort signal here.
            }
            "shutdown" => {
                cli.persist_session()?;
                break;
            ...
            other => {
                println!(
                    "{}",
                    json!({"type": "error", "message": format!("unknown command type: {other}")})
                );
                let _ = io::stdout().flush();
            }
        }
 ...
 ╚═════╝╚══════╝╚═╝  ╚═╝ ╚══╝╚══╝\x1b[0m \x1b[38;5;208mCode\x1b[0m 🦞\n\n\
  ...
  Type \x1b[1m/help\x1b[0m for commands · \x1b[1m/status\x1b[0m for live context · \x1b[2m/resume latest\x1b[0m jumps back to the newest session · \x1b[1m/diff\x1b[0m then \x1b[1m/commit\x1b[0m to ship · \x1b[2mTab\x1b[0m for workflow completions · \x1b[2mShift+Enter\x1b[0m for newline",
    ...
    fn run_prompt_ndjson(&mut self, input: &str) -> Result<(), Box<dyn std::error::Error>> {
        // Emit session_init as the first line so the caller knows the session ID.
        println!(
            "{}",
            json!({"type": "session_init", "session_id": self.session.id})
        );
        let _ = io::stdout().flush();
        ...
        let result = runtime.run_turn_with_callback(
            input,
            Some(&mut permission_prompter),
            Some(|event: &runtime::AssistantEvent| {
                let line = ndjson_line_for_event(event);
        ...
        // Final turn_end event with usage summary.
        println!(
            "{}",
            json!({
                "type": "turn_end",
...
fn sessions_dir() -> Result<PathBuf, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let path = cwd.join(".clawfirm").join("sessions");
...
/// Convert an [`AssistantEvent`] to a single NDJSON line for `--output-format ndjson`.
/// Returns `None` for events that carry no user-visible information.
fn ndjson_line_for_event(event: &runtime::AssistantEvent) -> Option<String> {
    let value = match event {
        runtime::AssistantEvent::TextDelta(text) if !text.is_empty() => {
            json!({"type": "text_delta", "text": text})
        }
        runtime::AssistantEvent::ToolUse { id, name, input } => {
            json!({"type": "tool_use", "id": id, "name": name, "input": input})
        }
        runtime::AssistantEvent::ToolResult(msg) => {
            // Flatten the first ToolResult block from the message.
            let block = msg.blocks.iter().find_map(|b| match b {
                ContentBlock::ToolResult {
                    tool_use_id,
                    tool_name,
                    output,
        ...
        runtime::AssistantEvent::PromptCache(_) | runtime::AssistantEvent::TextDelta(_) => {
            return None;
        }
    };
...
mod tests {
    ...
    fn default_permission_mode_uses_project_config_when_env_is_unset() {
        ...
        std::fs::create_dir_all(cwd.join(".clawfirm")).expect("project config dir should exist");
        std::fs::create_dir_all(&config_home).expect("config home should exist");
        std::fs::write(
            cwd.join(".clawfirm").join("settings.json"),
