...
pub enum AssistantEvent {
    ...
    /// Emitted after a tool has been executed; only fires via `run_turn_with_callback`.
    ...
    T: ToolExecutor,
{
    ...
    pub fn run_turn(
        ...
        self.run_turn_with_callback(user_input, prompter, None::<fn(&AssistantEvent)>)
    }
        ...
        F: Fn(&AssistantEvent),
    {
        ...
        loop {
            ...
            for (tool_use_id, tool_name, input) in pending_tool_uses {
                ...
                if let Some(cb) = &on_event {
                    cb(&AssistantEvent::ToolResult(result_message.clone()));
                }
...
> {
    ...
    for event in events {
        match event {
            ...
            // ToolResult is emitted by run_turn_with_callback after tool execution;
            // it will never appear in the raw API event stream fed to this function.
            AssistantEvent::ToolResult(_) => {}
        }
    }
