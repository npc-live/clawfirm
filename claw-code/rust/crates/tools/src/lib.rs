...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            name: "whipflow_run",
            description: "Execute a WhipFlow (.whip) workflow file. Use this when the user provides a .whip file path or asks to run a whipflow workflow.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "file": { "type": "string", "description": "Absolute path to the .whip file to execute." },
                    "source": { "type": "string", "description": "Inline .whip source code to execute (alternative to file)." },
                    "user_inputs": {
                        "type": "object",
                        "description": "Pre-filled answers for ask statements in the workflow.",
...
fn execute_tool_with_enforcer(
    ...
    match name {
        ...
        "whipflow_run" => from_value::<WhipflowRunInput>(input).and_then(run_whipflow),
...
fn run_whipflow(input: WhipflowRunInput) -> Result<String, String> {
    execute_whipflow(input)
}

fn execute_whipflow(input: WhipflowRunInput) -> Result<String, String> {
    let whip_binary = resolve_whip_binary().ok_or_else(|| {
        String::from("whip binary not found. Install with: go install github.com/ai-gateway/clawfirm/cmd/whip@latest")
    })?;

    let mut cmd = std::process::Command::new(&whip_binary);

    // Flags must come before the positional file argument.
    if let Some(inputs) = &input.user_inputs {
        for (key, value) in inputs {
            cmd.arg("-input").arg(format!("{key}={value}"));
        }
    }

    match (&input.file, &input.source) {
        (Some(file), _) => {
            let expanded = expand_tilde(file);
            if !std::path::Path::new(&expanded).exists() {
                return Err(format!("whip file not found: {expanded}"));
            }
            cmd.arg(&expanded);
        }
        (None, Some(source)) => {
            // Write source to a temp file and run it.
            let tmp_dir = std::env::temp_dir().join("whipflow");
            let _ = std::fs::create_dir_all(&tmp_dir);
            let tmp_file = tmp_dir.join("inline.whip");
            std::fs::write(&tmp_file, source).map_err(|e| format!("write temp whip: {e}"))?;
            cmd.arg(tmp_file);
        }
        (None, None) => {
            return Err(String::from("either 'file' or 'source' must be provided"));
        }
    }

    // Stream stdout line-by-line so session_progress events are processed
    // immediately as each session completes rather than waiting for the full run.
    // Null stdin so `ask` statements with missing inputs get EOF immediately
    // instead of blocking forever waiting for terminal input.
    cmd.stdin(std::process::Stdio::null());
    cmd.stdout(std::process::Stdio::piped());
    cmd.stderr(std::process::Stdio::piped());

    let mut child = cmd.spawn().map_err(|e| format!("failed to run whip: {e}"))?;
    ...
    // Progress file: written line-by-line so Go can tail it in real time.
    // Truncate on start so stale data from a previous run is cleared.
    let progress_path = std::env::temp_dir().join("whipflow-progress.ndjson");
    let _ = std::fs::write(&progress_path, b"");

    let mut log_lines: Vec<String> = Vec::new();
    let mut final_output_lines: Vec<String> = Vec::new();

    use std::io::BufRead;
    for line in std::io::BufReader::new(child_stdout).lines() {
        let line = line.map_err(|e| format!("read whip stdout: {e}"))?;
        let trimmed = line.trim();
        if trimmed.starts_with('{') {
            if let Ok(v) = serde_json::from_str::<serde_json::Value>(trimmed) {
                if v.get("type").and_then(|t| t.as_str()) == Some("session_progress") {
                    // Append raw JSON line to progress file so the Go process can
                    // tail it and emit EventToolExecutionUpdate in real time.
                    use std::io::Write;
                    if let Ok(mut f) = std::fs::OpenOptions::new().append(true).open(&progress_path) {
                    ...
                    let progress_line = if !done {
                        format!("▶ {label}{provider_str} starting...")
                    } else if !err.is_empty() {
                        format!("✗ {label} failed ({duration_ms}ms): {err}")
                    } else {
                        format!("✓ {label}{provider_str} done ({duration_ms}ms)")
                    };
                    eprintln!("{progress_line}");
                    log_lines.push(progress_line);
                    continue;
                }
            }
        }
        if !trimmed.is_empty() {
            final_output_lines.push(line.to_string());
        }
    }

    let status = child.wait().map_err(|e| format!("wait whip: {e}"))?;
    let stderr = stderr_thread.join().unwrap_or_default();

    if status.success() {
        let mut result = String::new();
        if !log_lines.is_empty() {
            result.push_str(&log_lines.join("\n"));
        }
        if !final_output_lines.is_empty() {
            if !result.is_empty() {
                result.push_str("\n\n");
            }
            result.push_str(&final_output_lines.join("\n"));
        }
        if result.trim().is_empty() {
            Ok(String::from("whipflow completed successfully (no output)"))
        } else {
            Ok(result)
        }
    } else {
        let mut msg = format!("whipflow failed (exit {}).", status.code().unwrap_or(-1));
        if !log_lines.is_empty() {
            msg.push_str(&format!("\n\nProgress:\n{}", log_lines.join("\n")));
        }
...
fn resolve_whip_binary() -> Option<String> {
    ...
    if let Ok(output) = std::process::Command::new("which").arg("whip").output() {
        if output.status.success() {
...
#[derive(Debug, Deserialize)]
struct WhipflowRunInput {
    file: Option<String>,
    source: Option<String>,
    user_inputs: Option<std::collections::BTreeMap<String, String>>,
}
...
fn resolve_skill_path(skill: &str) -> Result<std::path::PathBuf, String> {
    ...
    if let Ok(home) = std::env::var("HOME") {
        ...
        candidates.push(home.join(".clawfirm").join("skills"));
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".clawfirm").join("settings.local.json"),
    })
}
...
fn config_home_dir() -> Result<PathBuf, String> {
    ...
    Ok(PathBuf::from(home).join(".clawfirm"))
}
...
mod tests {
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".clawfirm")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".clawfirm")).expect("cwd dir");
        std::fs::write(
            home.join(".clawfirm").join("settings.json"),
            r#"{"verbose":false}"#,
        )
    ...
    fn enter_and_exit_plan_mode_round_trip_existing_local_override() {
        ...
        std::fs::create_dir_all(cwd.join(".clawfirm")).expect("cwd dir");
        std::fs::write(
            cwd.join(".clawfirm").join("settings.local.json"),
        ...
        let local_settings = std::fs::read_to_string(cwd.join(".clawfirm").join("settings.local.json"))
            .expect("local settings after enter");
        assert!(local_settings.contains(r#""defaultMode": "plan""#));
        let state =
            std::fs::read_to_string(cwd.join(".clawfirm").join("tool-state").join("plan-mode.json"))
